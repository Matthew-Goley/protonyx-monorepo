...
A **persistent NavBar** (`src/components/NavBar.tsx`, mounted app-wide in `App.tsx`, on every page except the auth route) sits fixed at the top: the wordmark on the left, and on the right exactly **two controls** - the account slot (`AccountMenu`, backed by the **real Fastify account system**) and a **two-bar hamburger** that crosses into an X and opens the full-screen `MenuOverlay`. **Every link lives in that overlay**, including the "Enter Lens Arc" CTA to `APP_URL`. The design is **Hairline** (winner of a 3-variant comparison; Float/Ledger and the switcher are deleted) and the bar is **theme-adaptive**: dark glass over `data-nav-dark` sections, light glass with dark text over white content (see §5).
...
Mounted once in `App.tsx`, fixed on every page except `/login` and `/signup`. The design is **Hairline**, winner of a 3-variant arrow-key comparison (Float and Ledger, the switcher, and the `lens_nav_variant` localStorage key were all deleted with the loss): a slim h-12 translucent bar with the Mercury-primary CTA.

**The bar carries exactly two controls, and that is a requirement rather than a style choice:** the `AccountMenu` slot (takes a `light` prop for its trigger colors) and the hamburger. Both are 36px tall so they share a baseline. The wordmark links home on the left. **No links are in the bar** - Engine and Referral program were inline here once (with gradient underline sweeps, since deleted), and the "Enter Lens Arc" CTA followed them into the menu. **Do not promote a link back into the bar as pages get added; add it to `MENU` in `content.ts` instead.**

The hamburger is **two bars, not three**: a 36px white tile with dark bars over dark sections, inverting to a dark tile with white bars over light ones, mirroring `frontend/`'s `.navbar-menu-button` and its `.navbar--light` override. While the menu is open the bars translate 3.5px toward each other and rotate ±45° into an X (`.menu-button.open` in `index.css`).

**The menu button must not move between open and closed, and three things conspire to keep it still.** It is worth understanding all three before touching this area, because each failure mode is invisible until someone watches the button:

1. **It is one element, not two.** The button is a *toggle*: the same node opens and closes the menu, so there is no separate close control that could sit anywhere else. The overlay has no header of its own for that reason.
2. **The bar stays mounted and on top of the panel** (`z-50` over the overlay's `z-40`), losing only its fill, border and blur while open, none of which affect layout. It does *not* fade out the way `frontend/` did, because a hidden bar forces the panel to supply its own close button, which reintroduces problem 1.
3. **`html { scrollbar-gutter: stable }`** (in `index.css`). This is the non-obvious one. Opening the menu locks body scroll, which reclaims the scrollbar's width and **widens the viewport by ~15px**, sliding every right-aligned fixed element left by exactly that much and back on close. Body padding cannot fix it, because fixed elements lay out against the viewport rather than the body box. Reserving the gutter permanently means the viewport width never changes at all. `overflow: hidden` still establishes a scroll container, so the gutter survives the lock. (Needs Safari 18.2+; older Safari gets the shift back.)

`dark` in the component is `sectionDark || menuOpen`, since an open panel puts `#0c0f16` under the bar whatever section it was over.

### The menu overlay (components/MenuOverlay.tsx + the .menu-overlay block in index.css)

A full-bleed dark panel ported in style from `frontend/chrome.css`: columns of oversized links (`clamp(1.6rem, 3vw, 3rem)`) that **wipe in left-to-right on a staggered delay** (`clip-path: inset(0 100% 0 0)` -> `inset(0)` plus opacity and an 8px rise, 0.08s then +0.06s per link). Structure lives in the component; every animation lives in `index.css`.

- **It has no header of its own.** The NavBar sits on top of it and supplies both the wordmark and the close control (see the nav's point 1 above); the panel just pads past the bar. The original's wordmark + white close square are gone for that reason.
- **It stays mounted and toggles `.open`**, as the original did. A conditionally rendered element has nothing to transition *from*, so unmounting would kill both the fade and the exit. While closed it is `visibility: hidden` + `pointer-events: none`, plus `aria-hidden` and `inert` so it is out of the accessibility tree and the tab order.
- **The stagger relies on `nth-of-type` counting `<a>` siblings only**, so the column's `<p class="menu-label">` heading does not consume a delay slot and each column starts its own cascade. Keep links as direct `<a>` children of `.menu-column`. (Vite's minifier rewrites `:nth-of-type(1)` to `:first-of-type` in the build output; same thing.)
- **`MENU` in `content.ts` is the source of truth for its links**, currently two columns: Navigation (Home, Engine, Referral program) and Legal (Terms of Service, Privacy Policy). `/login` and `/signup` are deliberately absent from the columns because the account slot and the menu's own auth action already lead there.
- The bottom row carries **the "Enter Lens Arc" CTA** (a Mercury primary `BtnLink` to `appOpenUrl(user?.email)`; **this is the only place on the site chrome that CTA lives now**, so do not delete it without moving it) and the **auth action**, styled like the original's `.menu-logout-button` (small, uppercase, wide-tracked, quiet until hover): "Log out" when signed in, otherwise a "Sign in" link carrying the current path as `?next=`.
- Closes on the menu button, on a backdrop click (guarded on `e.target === e.currentTarget` so a click on a link or the panel's padding does not count), and on Escape. Body scroll is locked while open and the previous value restored on close; `overscroll-behavior: contain` stops a wheel or touch scroll chaining through to the page behind.
...
- **A new page must be added to `MENU` in `content.ts` in the same change that adds its route to `App.tsx`.** The nav bar carries no links at all any more, so the menu overlay is the only place the site links to itself; a route that is not in `MENU` is reachable only by typing the URL. Do not "fix" that by putting the link back in the bar (see §5 The nav).
- **`html { scrollbar-gutter: stable }` in `index.css` is load-bearing, not cosmetic.** It exists so that locking body scroll for the menu cannot change the viewport width and shift the fixed nav controls sideways. Removing it reintroduces a ~15px jump of the menu button on every open and close. See §5 The nav, point 3.
