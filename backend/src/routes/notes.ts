...
export default async function noteRoutes(app: FastifyInstance) {
    // Notes (protected)
    app.post("/notes", { preHandler: authenticate }, async (request: any, reply: any) => {
    ...
    // Get Notes (protected)
    app.get("/getnotes", { preHandler: authenticate }, async (request: any, reply: any) => {
    ...
    app.delete("/notes/:id", { preHandler: authenticate }, async (request: any, reply:any) => {
    ...
    app.patch("/notes/:id", { preHandler: authenticate }, async (request: any, reply: any) => {
        const { title, content } = request.body as {
            title: string,
            content: string
        };
