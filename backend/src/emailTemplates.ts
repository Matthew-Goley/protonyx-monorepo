// Inline HTML strings for transactional emails. Kept out of email.ts so the
// send logic there stays scannable. The HTML uses table-based layout for
// broad email-client compatibility and mirrors the brand palette: dark navy
// background (#0b1020), warm-white text (#e7ebf3), teal CTA (#2dd4bf).

export function welcomeEmailHtml(username: string, downloadUrl: string): string {
    return `<!DOCTYPE html>
<html>
...
export function verifyEmailHtml(username: string, verifyUrl: string): string {
    return `<!DOCTYPE html>
<html>
  <body style="margin:0;padding:0;background-color:#0b1020;font-family:'IBM Plex Mono',ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,monospace;color:#e7ebf3;">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#0b1020;">
