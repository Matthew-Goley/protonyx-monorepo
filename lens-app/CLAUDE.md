...
- a widget grid (Caution Score, Portfolio Vector, Positions, Total Equity, Sharpe, Composition, Beta, Dividend Calendar),
...
- `lens_layout` — JSON array of `LayoutItem` (`{ widgetId, x, y, w, h }`, grid-cell units) for the dashboard widget grid. Default when unset/malformed is `getDefaultLayout()` (see the widget grid below). **No UI mutates this yet** — the accessors exist so the future drag/edit phase gets persistence for free.

Accessors: `getPositions()`, `setPositions()`, `getSettings()`, `setSettings()`, `getLayout()`, `setLayout()`. All are defensive (malformed JSON returns the empty/default; `getLayout` also validates each item's shape before trusting the array). `RiskTier` and `StoredSettings` types are exported here; `LayoutItem` comes from `@/lib/widgetLayout`.
...
The dashboard renders a **data-driven** 12-column square-cell grid, not a hardcoded widget list. Row spans are **measured at runtime**, not guessed: the grid measures each widget's real rendered height and computes the whole-number row span that contains it, so it self-corrects for **any** portfolio (more Positions rows, longer brief, more sectors) with **zero manual span tuning**. It is the foundation for a later phase (add-widget menu + Apple-style drag/edit with auto-reflow); that phase is additive because the coordinate + packer + square-cell model below is unchanged — the only new thing is where `h` comes from.

- **`widgetRegistry.tsx`** — the single source of truth every grid phase reads from. `WIDGET_REGISTRY` is an ordered array of `{ id, title, render: (r: LensResult) => ReactElement, defaultSpan: {w,h}, minSpan?, maxSpan?, defaultVisible }`, one entry per widget. `render` wraps the finalized widget component (`<XWidget result={r} />`), so the grid never touches widget internals. **`defaultSpan.w` is the authoritative width; `defaultSpan.h` is now a MINIMUM FLOOR** ("never render shorter than this", to keep intentional shapes like the caution-score square) — measurement raises it as needed, so floors are set low: `caution-score 3×3, total-equity 6×2, composition 4×3, portfolio-momentum 3×3, positions 5×3, sharpe 3×2, beta 3×2, dividend-calendar 4×2`. `title`/`minSpan`/`maxSpan`/`defaultVisible` are declared now but unused this phase (data-ready for the add-menu + resize). `getWidget(id)` looks up an entry. It exports **data + a render map only, never a component** (keeps oxlint `react/only-export-components` happy). **Registry order == default dashboard order** (fed to the packer); the widths tile 12 columns into three full-width bands (`[caution | total-equity | sharpe]`, `[composition | positions | portfolio-momentum]`, `[beta | dividend-calendar]`), each band's height set by measurement.
- **`widgetLayout.ts`** — grid geometry + layout model + fit math. `GRID_COLUMNS = 12` (clean divisibility: 2/3/4/6) and `GRID_GAP = 24` (px, matches the §6 `gap-6` gutter) are the single source of grid size; everything derives from them. `LayoutItem = { widgetId, x, y, w, h }` (cell units). `placeWidgets(footprints, columns)` is a deterministic first-fit packer (scan row by row, left to right; place each widget in the first free non-overlapping slot that fits without running past the right edge). The **pure fit helpers** are the whole clip-avoidance calculation, kept side-effect-free so they are trivially testable: `widgetPxWidth(w, cellSize, gap) = w*cellSize + (w-1)*gap`; `rowsForHeight(contentPx, cellSize, gap) = ceil((contentPx + gap) / (cellSize + gap))` (smallest N whose stacked height contains `contentPx`); `fitSpan(floorH, contentPx, cellSize, gap) = max(floorH, rowsForHeight(...))`. `getDefaultLayout()` (used only by the dormant cookie, see below) runs the floors through the packer, computed **lazily** so the `cookies → widgetLayout → widgetRegistry → DashboardWidgets → cookies` import cycle stays safe. Dense upward compaction is a later refinement, not now.
...
- **`WidgetGrid.tsx`** (`{ result }`) — **two-pass measure-then-place** (see §7 for the full mechanics): PASS 1 renders every visible widget in a hidden off-screen layer at its true column pixel width (auto height) and measures each natural content height; COMPUTE turns those into footprints `{ w: defaultSpan.w, h: fitSpan(defaultSpan.h, measuredPx, cellSize, gap) }` and runs the existing packer; PASS 2 renders the real grid at the computed coordinates, each cell an `h-full w-full [&>*]:h-full` wrapper that stretches the widget's **outer Panel** to fill its (now correctly sized) cell **without editing the widget**. The grid sets **no fixed height**, so it grows with content and inherits the AppShell `<main>` scroll region — a vertical scrollbar appears only when the page exceeds the viewport, never always-on.

**The `lens_layout` cookie is dormant this phase.** `getLayout`/`setLayout` exist for the future drag phase, but `WidgetGrid` does **not** read the cookie: a persisted layout would carry a stale measured `h` once content height changes, so placement stays **live-computed on every load** (and on every `cellSize`/`result` change) until drag writes explicit user layouts.
...
| **Dashboard** | `pages/Dashboard.tsx` | Redirects to `/onboard` when no positions cookie. If `!isSubscribed(user)` renders `<UpgradePrompt/>` instead of data. Otherwise: `PositionActions` box + Lens Brief panel (brief text + an "Analysis" link to `/analysis`), then `<WidgetGrid result={result} />` — the data-driven 8-widget grid (registry + layout cookie, see §5). Loading shows static skeletons; error shows a retry panel. The three header icon buttons (Plus / Trash2 / Pencil) are non-functional placeholders for the future add-menu / delete / edit-mode. Position add/remove lives in `PositionActions`, Onboard and Settings (unrelated to widget management). |
...
- `components/dashboard/PositionActions.tsx` — `PositionActions`, a compact vertical action box sitting to the **left of the Lens Brief** on the Dashboard: a big gradient **+** (opens `AddPositionModal`) and a **Manage** button (slider icon) that opens a right slide-over **drawer** for editing share counts / deleting holdings. Driven by `usePositionsManager` (`src/hooks/usePositionsManager.ts`), which persists add/edit/delete to the `lens_positions` cookie and invalidates `['lens-analysis']`. The manager is created in `Dashboard` (not in `PositionActions`) so a mutation re-renders the page and `useLensAnalysis` refetches its keyed-by-positions query. This is **position** management, unrelated to widget management.
- `components/dashboard/WidgetGrid.tsx` — `WidgetGrid` (`{ result }`), the data-driven dashboard grid renderer, **two-pass measure-then-place** so row spans are never hardcoded guesses. **PASS 1 (measure):** a hidden layer (`position:absolute; left:-9999px; visibility:hidden; pointer-events:none; aria-hidden`, kept in layout so it actually measures) renders every visible widget's `render(result)` at exactly `widgetPxWidth(defaultSpan.w, cellSize, gap)` wide with **auto height**; each box's `getBoundingClientRect().height` goes into a ref map keyed by widget id. **COMPUTE:** `finalH = fitSpan(defaultSpan.h, measuredPx, cellSize, gap)`, `finalW = defaultSpan.w`, then the existing `placeWidgets` packer (registry order) yields x/y — done in a **`useLayoutEffect` keyed on `[cellSize, result]`** so the computed layout commits before paint (no flicker) and re-measures on resize (cell height changes) or content change; guarded on `cellSize > 0` (renders a static dim `GridSkeleton` until then). **PASS 2 (place):** the real grid renders that layout at explicit `gridColumn`/`gridRow`, each cell an `h-full w-full [&>*]:h-full` wrapper stretching the outer Panel to fill the correctly-sized cell without editing the widget. A **DEV-only** `useEffect` (`import.meta.env.DEV`) is a standing regression guard: it `console.warn`s any widget whose measured content exceeds its cell height by >1px (`"<id> CLIPS: content Xpx > cell Ypx"`) and any AABB overlap the packer produced. It never fires when the fit math is right and is stripped from prod. Does **not** read the `lens_layout` cookie (placement is live-computed; see §5). Backed by `@/lib/widgetRegistry`, `@/lib/widgetLayout` (incl. the pure `widgetPxWidth`/`rowsForHeight`/`fitSpan` helpers), `@/hooks/useGridMetrics`.
...
New page or route -> §4/§7. New widget (add a registry entry with a low `h` floor; measurement sizes it) or derived metric -> §5/§7. Change to the grid model (columns, gap, packer, fit helpers, measure/place passes, layout cookie, cell sizing) -> §5/§7. New design token -> §6. New lens-api method or changed response -> §8 (+ `../lens-api/CLAUDE.md`). New Fastify call or auth change -> §4/§9 (+ `../CLAUDE.md`). New dependency or build/lint change -> §2/§3. Treat the doc as part of the diff.
