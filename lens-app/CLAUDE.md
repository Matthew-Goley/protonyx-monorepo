...
| **Settings** | `pages/Settings.tsx` | General (Theme/Date format dropdowns — **display-only, not persisted**), Investment Style (`RiskProfileCards`, persists + invalidates query), Subscription (Pro badge + "Manage Billing" via `POST /stripe/portal` when subscribed, else "Upgrade to Lens Pro" via `POST /stripe/create-checkout-session`), a functional **Data & Refresh** section ("Clear Data & Restart Onboarding" → `clearAllData()` wipes the positions/settings/layout cookies then redirects to `/onboard`), five **placeholder** `Collapsible` sections ("coming soon": Portfolio Direction Thresholds, Volatility, Lens Signal Thresholds, Monte Carlo, Developer), Positions CRUD (add/remove, "Edit" is disabled), and an About section that pings `lensApi.health()` for live API status (with the app version, see §10). |
