...
from templates import login_link_html, magic_link_html
...
def send_magic_link(to: str, verify_url: str, returning: bool = False) -> None:
    """Send the magic-link email. `returning=True` is a verified email hitting
    /join again (they're trying to get back in, not sign up) - same token/verify
    mechanics, different copy so it doesn't read like a second confirmation."""
    ...
    subject = "Your Lens Arc login link" if returning else "Confirm your Lens Arc early access"
    html = login_link_html(verify_url) if returning else magic_link_html(verify_url)
    try:
        resend.Emails.send(
            {
                "from": FROM_ADDRESS,
