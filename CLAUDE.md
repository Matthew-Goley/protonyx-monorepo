...
│   │   ├── hooks/                # useLensAnalysis.ts (react-query POST /analyze, shared cache), usePortfolioHistory.ts (real equity curve from batched /tickers/history), useGridMetrics.ts (ResizeObserver → square cell size), usePositionsManager.ts (holdings CRUD → cookie + query invalidate)
│   │   ├── components/
