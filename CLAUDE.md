...
| `GET` | `/me` | ✅ | — | Returns the full user profile **excluding `password`, `stripe_customer_id`, `verification_token`, `tos_accepted_at`, and `eula_accepted_at`**. Shape: `{ success, user: { id, username, email, plan, plan_expires_at, member_since, last_login, beta_access, download_count, email_verified, is_active, tos_version_accepted, eula_version_accepted, subscription_status, risk_tier, settings } }`. (`tos_version_accepted` and `eula_version_accepted` are included so the client can reason about legal state; the `*_accepted_at` timestamps are deliberately omitted. `risk_tier` is the user's onboarding risk profile, `null` until onboarding sets it. `settings` is the per-user prefs JSONB blob, `{}` until the client writes any preference.) The frontend calls this on login and on every account-page load. |
