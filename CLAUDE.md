...
Note that the months formula here (`min(1 + referrals, 12)`) is deliberately **not** the step function in `referral-service/entitlement.py` (`MILESTONES`: 1/2/4/6/lifetime). The two are independent grant paths and are currently out of lockstep; if they are meant to agree, reconcile them explicitly rather than assuming either one is authoritative.
